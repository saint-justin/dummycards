...
export default (props:WidgetDropdown): React.ReactElement  => {
  ...
  const generateOptions = (options: string[]) =>  options.map((opt: string):JSX.Element => <option value={opt.toLowerCase()} key={opt}>{opt}</option>);
  ...
  return (
    <>
      ...
      <div className='flex-row'>
        <select id={'select_' + cleanName(props.name)} onChange={handleSelectChange}>
